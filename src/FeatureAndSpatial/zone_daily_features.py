...
def build_zone_daily_features(zone_hour_df: DataFrame) -> DataFrame:
    """
    Build zone_daily_features from zone_hour_features.

    One row = one pickup zone on one pickup_date.
    We aggregate hourly totals upward to daily totals, then
    recompute daily averages from totals / trip_count.
    """
    # 先按 zone + date 聚合 totals 和 counts
    daily_df = (
    ...
    # 用 totals / trip_count 重新计算 daily averages
    daily_df = (
        daily_df
        .withColumn(
            "avg_trip_distance",
    ...
    return reorder_columns(daily_df)
...
def export_sample_csv(zone_daily_df: DataFrame) -> None:
    """
    ...
    try:
        ...
        sample_path = TABLES_DIR / "zone_daily_features_sample.csv"
...
def main() -> None:
    ...
    zone_daily_df = build_zone_daily_features(zone_hour_df)
